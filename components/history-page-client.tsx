...
import { historyListCacheKey, rememberPersonalListUserId, getLastPersonalListUserId } from "@/lib/personal-list-cache";
import { usePersonalList } from "@/hooks/use-personal-list";
import type { PaginatedReadingHistoryResult } from "@/lib/actions/reading-progress.actions";
...
export function HistoryPageClient() {
  ...
  const cacheKey = effectiveUserId
    ? historyListCacheKey(effectiveUserId, requestedPage)
    : null;

  const fetcher = useCallback(
    () => getMyReadingHistoryPage(requestedPage, ITEMS_PER_PAGE),
    [requestedPage],
  );

  const { data, isLoading, error } = usePersonalList<PaginatedReadingHistoryResult>(
    {
      cacheKey,
      fetcher,
      // Only hit the network once we know the real session user.
  ...
  const emptyResult = useMemo<PaginatedReadingHistoryResult>(
    () => ({
      items: [],
      page: requestedPage,
  ...
  const readingHistory = result.items;
